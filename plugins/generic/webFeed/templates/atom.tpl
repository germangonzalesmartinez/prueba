...
<feed xmlns="http://www.w3.org/2005/Atom">
	...
	{foreach name=sections from=$publishedSubmissions item=section key=sectionId}
		{foreach from=$section.articles item=article}
			<entry>
				{* required elements *}
				<id>{url page="articles" op="view" path=$article->getBestId()}</id>
				...
				<link rel="alternate" href="{url page="articles" op="view" path=$article->getBestId()}" />

				{if $article->getLocalizedAbstract()}
					<summary type="html" xml:base="{url page="articles" op="view" path=$article->getBestId()}">{$article->getLocalizedAbstract()|strip|escape:"html"}</summary>
				{/if}
